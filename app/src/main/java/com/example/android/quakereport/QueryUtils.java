...
public final class QueryUtils {
   ...
   // public final class QueryUtils {
        ...
        /**
         * Return a list of {@link EqAdapter} objects that has been built up from
         * parsing a JSON response.
         */
        public static ArrayList<clsEQData> extractEarthquakes() {

            // Create an empty ArrayList that we can start adding earthquakes to
            ArrayList<clsEQData> earthquakes = new ArrayList<>();
            ...
            try {
                ...
                JSONObject JMainStr = new JSONObject(SAMPLE_JSON_RESPONSE); //turn string into jason object
                JSONArray Jfeatures = JMainStr.getJSONArray("features"); // get features array from Json object
                // now loop through all element in array
                for (int i=0;i<Jfeatures.length();i++){
                    JSONObject JObj = Jfeatures.getJSONObject(i);
                    JSONObject JProp = JObj.getJSONObject("properties");
                    // change to 0.0 format
                    DecimalFormat formatter = new DecimalFormat("0.0");
                    String mMag = formatter.format(JProp.getDouble("mag")).toString();
                    String mLog = JProp.getString("place");
                    //Split into two sections
                    int ptr = mLog.indexOf(" of ");
                    String FirstLoc;
                    String SecondLoc;
                    if (ptr ==-1) {
                        FirstLoc = "Near the";
                    ...
                    {
                        ptr=ptr+4;
                        FirstLoc =  mLog.substring(0, ptr).trim();
                        SecondLoc = mLog.substring(ptr, mLog.length() ).trim();
                    }


                    // get date and format it
                    Date dateObj = new Date(JProp.getLong("time"));
                    SimpleDateFormat dateFormat = new SimpleDateFormat("MMM DD, yyyy");
                    String mDate = dateFormat.format(dateObj);
                    //now format the time
                    SimpleDateFormat TimeFormat = new SimpleDateFormat("h:mm a");
                    String mTime = TimeFormat.format(dateObj);


                    clsEQData EqDataTmp = new clsEQData(mMag, FirstLoc,SecondLoc, mDate,mTime);
                    earthquakes.add(EqDataTmp);
